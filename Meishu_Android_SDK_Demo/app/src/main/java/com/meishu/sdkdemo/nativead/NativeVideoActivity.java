...
public class NativeVideoActivity extends AppCompatActivity {
        ...
        NativeMediaAdListener listener = new NativeMediaAdListener() {
            ...
            public void onAdLoaded(List<NativeMediaAdData> mediaAdDatas) {
                if (mediaAdDatas != null && mediaAdDatas.size() > 0) {
                    ...
                    initADUI();
                    if (mMediaAdData.getAdPatternType() == MeishuAdPatternType.VIDEO) {
                        mMediaAdData.preLoadVideo();
                        textLoadResult.setText("getAdPatternType() == MeishuAdPatternType.VIDEO：" + "这是一条视频广告");
                    } else if (mMediaAdData.getAdPatternType() == MeishuAdPatternType.IMAGE && mMediaAdData.getImgList() != null && mMediaAdData.getImgList().size() == 1) {
                        /**
                         * 如果该条原生广告只是一个普通图文的广告，不带视频素材，那么渲染普通的UI即可。
                         */
                        textLoadResult.setText("getAdPatternType() == MeishuAdPatternType.IMAGE：" + "这是一条两图两文广告");
                    } else if (mMediaAdData.getAdPatternType() == MeishuAdPatternType.IMAGE && mMediaAdData.getImgList() != null && mMediaAdData.getImgList().size() == 3) {
                        textLoadResult.setText("getAdPatternType() == MeishuAdPatternType.IMAGE：" + "这是一条三小图广告");
                    }
                }
            }
            ...
            @Override
            public void onADExposure(NativeMediaAdData mediaAdData) {
    ...
    private void initADUI() {
        ...
        if (patternType == MeishuAdPatternType.IMAGE || patternType == MeishuAdPatternType.VIDEO) {
        ...
        } else if (patternType == MeishuAdPatternType.IMAGE && mMediaAdData.getImgList() != null && mMediaAdData.getImgList().size() == 3) {
    ...
    private void bindMediaView() {
        if (mMediaAdData.getAdPatternType() == MeishuAdPatternType.VIDEO) {
            ...
            /** 设置视频播放过程中的监听器 */
            mMediaAdData.setMediaListener(new NativeAdMediaListener() {
                @Override
